...
# irisデータセットを用いて最適n_min決定のためのCVを実行。
# ここでは損失関数としてmis_match()を採用した場合の例を示す。
df <- iris
n <- nrow(df)
p <- ncol(df)
index <- sample(1:n, n, replace = FALSE) # irisデータは応答変数値でソートされているので、乱雑化する。
...
for (n_min in n_min_seq) {
  ...
  for (h in 10:1) {
    ...
    vertex <- dt(x[train, ], y[train], "mis_match", n_min, p)
    for (i in test) {
      SS <- SS + as.integer(value(x[i, ], vertex) != y[i])
    }
  }
  print(SS)
  out <- rbind(out, c(n_min, SS))
}
print(out)