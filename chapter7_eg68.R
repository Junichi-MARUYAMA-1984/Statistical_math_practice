...
for (m in 4:1) {
  for (b in 1:B) {
    ...
    vertex <- dt(x[index, ], y[index], "mis_match", n_min = 2, m = m)
    for (i in test) {
...
lines(1:B, rf(res_m[[2]]) + 0.1, col = 4)
lines(1:B, rf(res_m[[1]]) - 0.1, col = 5)
legend("bottomright",
       legend = c("m = 4", "m = 3", "m = 2", "m = 1"),
       col = c(2, 3, 4, 5),
       lty = 1)
